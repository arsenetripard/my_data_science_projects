...
starting_date = st.sidebar.date_input('Starting date: ', datetime.datetime(2021, 5, 21))
end_date = st.sidebar.date_input('End date: ', datetime.datetime(2022, 12, 20)) 
...
if upload is not None:
    ...
    with c2: 
        st.metric( "Number of servers", value=4311)
        ...
        plt.xticks(graph1.index[::45], rotation=30) 
        st.pyplot(fig)
    ...
    with c1:
        ...
        st.text("")
        st.text("")
        c1.metric(  "Global Emission (TeqCO2)", 
    ...
    with c2:
        ...
        st.text("")
        st.text("")
        c2.metric(  "CPUs Emission (TeqCO2)", 
    ...
    with c3: 
        ...
        st.text("")
        st.text("")
        c3.metric(  "Memory Emission (TeqCO2)", 
    ...
    with c2: 
        graph_title = '<p style="font-size: 22px; text-align: center;">Evolution of Power Consumption and Gas Emission</p>'
        st.markdown(graph_title, unsafe_allow_html=True)  
        ...
        ax2.set_ylabel('Global Emission (TeqCO2)')
